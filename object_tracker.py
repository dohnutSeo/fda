...
def main(_argv):
    ...
    while True:
        ...
        for track in tracker.tracks:
            if not track.is_confirmed() or track.time_since_update > 0.5:
                continue 
            bbox = track.to_tlbr()
        ...
        # draw bbox on screen
            ...
            cv2.rectangle(frame, (int(bbox[0]), int(bbox[1]-30)), (int(bbox[0])+(len(class_name))*10, int(bbox[1])-12), color, -1)
            cv2.putText(frame, class_name ,(int(bbox[0]), int(bbox[1]-10)),0, 0.5, (255,255,255),1)
            
            ###################################
            # Distance Calculation 2021/12/28 #
            ###################################
            #               car :3                bus                truck
            if class_name == "vehicle" or class_name == "pothole" or class_name == "pedestrian":
                ...
                if apx_distance <= 7:
                    if (mid_x) > width*0.45 and (mid_x) < width*0.53:
                        # cv2.putText(frame, 'WARNING!!!', (400,150), cv2.FONT_HERSHEY_SIMPLEX, 5, (0,0,255), 4)
                        # cv2.putText(frame,  class_name + str(track.track_id), (400,220), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 3)
                        ps.putBText(frame, 'WARNING!!! : '+class_name, 400, 150, vspace=1,
                             hspace=1, font_scale=1, background_RGB=(228, 225, 222), text_RGB=(255, 0, 0))
